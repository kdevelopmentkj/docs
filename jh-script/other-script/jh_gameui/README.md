...
* **MapLibre GL map** — vector tiles, rotatable bigmap, GPS route rendering, smooth pan / zoom, style switcher.
...
* **Subscription-based widgets** — each widget (speedometer, compass, map, stats…) subscribes only to the Lua streams it needs. Idle widgets are silent, no polling runs in the background.
...
* **State-bag-driven server info** — `GlobalState.JH_GameUI_PlayersCounts` / `GlobalState.JH_GameUI_maxPlayers` feed the player-count widget.
* **i18n** — English & French bundled, easily extensible via the NUI `app/locales` folder (NUI source only).
...
Only `config.lua` is unlocked (`escrow_ignore`). All other Lua files and the built NUI bundle are shipped encrypted. The public integration surface is documented below ([client.md](client.md)) and is stable across updates.
